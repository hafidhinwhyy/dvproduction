<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			extract($_POST); 
			$q = mysql_query("update pesanan SET nama='$nama',tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',
			user_id='$user_id',alamat='$alamat',telephone='$telephone' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("pesanan.php"); }
		}
	}
...
?> 
	<div class="container">
		<?php
			...
			$j = mysql_num_rows($q);
		?>
		<h4>Pesanan Masuk (<?php echo ($j>0)?$j:0; ?>)</h4>
		<hr>
		<?php
			if(!empty($_GET)){ 
				if($_GET['act'] == 'edit'){
