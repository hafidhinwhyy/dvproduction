<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			extract($_POST); 
			$password = md5($password);
			$q = mysql_query("update user SET nama='$nama',email='$email',telephone='$telephone',
			alamat='$alamat',password='$password',status='$status' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
...
?> 
	<div class="container">
		<?php
			...
			$j = mysql_num_rows($q);
		?>
		<h4>Daftar User (<?php echo ($j>0)?$j:0; ?>)</h4>
