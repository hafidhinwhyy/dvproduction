						...
						<?php
							if($_GET['act'] == 'bayar' && $_GET['id']){
								if(!empty($_POST)){
									...
									extract($_POST);
									$q = mysql_query("insert into pembayaran Values(NULL,'$_GET[id]','$_SESSION[iam_user]','$gambar','$bayar','pending',NOW())");
									if($q){
						...
						<tbody>
							<?php while($data=mysql_fetch_object($q)){ ?>
								<tr <?php if($data->read == 0 ){ echo 'style="background:#cce9f8 !important;"'; } ?> >
									...
									<td>
										<a class="btn btn-sm btn-success" href="pembayaran.php?act=bayar&id=<?php echo $data->id; ?>">Bayar</a>
